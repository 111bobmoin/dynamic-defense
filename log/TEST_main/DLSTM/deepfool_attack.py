...
def deepfool_attack(model, X, y, epsilon=0.1, maxiter=50):
...
# ========== 生成对抗样本并保存 ==========
def generate_and_save_deepfool_adversarial(model_path, raw_data, labels, output_X_path, output_y_path):
    ...
    print("Generating adversarial examples with deepfool...")

    X_numpy = raw_data.squeeze(1)  # (N, 29)
    y_numpy = labels

    adv_examples = deepfool_attack(model, X_numpy, y_numpy)
    ...
    print(f"deepfool adversarial data saved to: {output_X_path}, {output_y_path}")
...
if __name__ == "__main__":
    ...
    for i in range(1, 41):
        ...
        x_path = f'deepfoold_data/deepfool_X_train{i}.txt'
        y_path = f'deepfool_data/deepfool_y_train{i}.txt'
    ...
    # 生成并保存对抗样本
    """generate_and_save_deepfool_adversarial(
        model_path='lstm_model.pth',
        raw_data=torch.tensor(raw_data, dtype=torch.float32),
        labels=labels,
        output_X_path='deepfool_data/deepfool_X_train.txt',
        output_y_path='deepfool_data/deepfool_y_train.txt'
    )"""
