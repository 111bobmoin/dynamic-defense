...
def fgsm_attack(model, X, y, epsilon=0.1, maxiter=50):
...
# ========== 生成对抗样本并保存 ==========
def generate_and_save_fgsm_adversarial(model_path, raw_data, labels, output_X_path, output_y_path):
    ...
    adv_examples = fgsm_attack(model, X_numpy, y_numpy)
    ...
    print(f"FGSM adversarial data saved to: {output_X_path}, {output_y_path}")
...
if __name__ == "__main__":
    ...
    for i in range(1, 41):
        flipped_labels = adversarial_label_flip(raw_data, labels, flip_ratio=0.5)

        x_path = f'FGSM_data/FGSM_X_train{i}.txt'
        y_path = f'FGSM_data/FGSM_y_train{i}.txt'
    ...
    # 生成并保存对抗样本
    """generate_and_save_FGSM_adversarial(
        model_path='lstm_model.pth',
        raw_data=torch.tensor(raw_data, dtype=torch.float32),
        labels=labels,
        output_X_path='FGSM_data/FGSM_X_train.txt',
        output_y_path='FGSM_data/FGSM_y_train.txt'
    )"""
