...
precision_origin = test_model(model_path, data_path, label_path)
print("开始50轮FGSM攻击:")
print(f"-------------------------------------------------------------------------------------------")

for i in range(1, 41):
    print(f"第{i}次FGSM攻击")
    print(f"FGSM攻击后异构体K-means:")
    # 获取三个模型的预测结果
    model_path = f"./Kmeans/all_kms_model/bad_kms_model/hop_kms_model_{i+80}.pth"
    data_path = "./Kmeans/test_data/X_test.txt"
    label_path = "./Kmeans/test_data/y_test.txt"
    attack_precision = test_model(model_path, data_path, label_path)
    ...
    y_true = np.loadtxt("./DLSTM/test_data/y_test.txt")  # 修改为你的真实标签来源 C:/Users/xiaowangPC/Desktop/log/TEST_main/
