...
for i in range(1, 31):
    print(f"第{i}轮FGSM攻击")
    print(f"FGSM攻击后异构体双向LSTM:")
    model_path_FGSM = f'./DLSTM/FGSM_lstm_model/FGSM_lstm_model{i}.pth'
    test_data_file = './DLSTM/test_data/X_test.txt'
    label_file = './DLSTM/test_data/y_test.txt'

    X, y = load_and_preprocess_data(test_data_file, label_file)
    precision = predict_and_evaluate(X, y, model_path_FGSM)
    ...
    y_pred2 = predict_and_evaluate(X, y,
                                   model_path=f'./DLSTM/FGSM_lstm_model/FGSM_lstm_model{i}.pth')
    config = Config()
