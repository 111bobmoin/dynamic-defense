...
def eval(attack, index, precision):
    ...
    # 第2个模型
    model2 = LSTMModel(16)
    model2.load_state_dict(torch.load(f'models/attack_lstm_{attack}_{index}.pth'))
    model2.eval()
    y_pred2 = []
    with torch.no_grad():
        for X_batch, y_batch in val_loader:
    ...
    model3.load_state_dict(torch.load('models/origin_ag.pth'))
    model3.eval()
    y_pred3 = []
    ...
    y_pred = []
    be_attack = show.report2(y_true, y_pred2, class_name)
    print("攻击后异构体 : ", end=" ")
...
def dhr(attack):
    model = LSTMModel(16)
    start = template_eval_lstm.eval2(model, "models/origin_lstm.pth", "Dataset/validata.csv")
    cnt1, cnt2 = 0, 0
    print("原始异构体: ")
