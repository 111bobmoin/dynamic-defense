...
def eval(attack, index, precision):
    ...
    # 固定为16个子类别
    # 第1个模型
    model1 = AutoregressiveModel(16)
    model1.load_state_dict(torch.load('models/origin_ag.pth'))
    model1.eval()
    y_pred1 = []
    y_true = []
    with torch.no_grad():
    ...
    # 第3个模型
    model3 = SubspaceClusteringModel(16)
    model3.load_state_dict(torch.load(f'models/attack_subplace_{attack}_{index}.pth'))
    model3.eval()
    y_pred3 = []
